...
红色部分是$sum_i$，橙色部分是$sum1_i$，可以发现$sum_i+sum1_{son_i}=sum_1=sum1_{rt}$

所以答案变成：

$ans_{rt}=ans+\sum_{i\in (1,rt)}{sum1_i}^2$

$=ans+{sum1_{rt}}^2+\sum_{i\in (1,fa_{rt})}{sum1_i}^2-\sum_{i\in (1,rt)}{sum_i}^2$
